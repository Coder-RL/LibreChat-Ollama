...
"""
    ...
    def format_context(self, chunks: List[Dict[str, Any]]) -> str:
        """
        Format code chunks into a context string for the LLM prompt.
        ...
        try:
            if not chunks:
                logger.warning("[CTX] No chunks provided for context formatting")
                return ""

            # Format the chunks with metadata
            formatted_context = self.format_chunks(chunks, include_metadata=True)
...
"""
            ...
            logger.info(f"[CTX] Formatted {len(chunks)} chunks into context ({len(context)} chars)")
            return context

        except Exception as e:
            logger.error(f"[CTX] Error formatting context: {str(e)}")
            return ""
