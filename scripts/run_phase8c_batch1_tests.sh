...
if [[ "$health" == *"postgresql"* && "$health" == *"opensearch"* ]]; then
...
echo "🚀 Running Phase 8C Batch 1 tests..."

# Run the test using unittest with verbose output
cd /Users/robertlee/GitHubProjects/ollama-inference-app
python -m unittest tests/test_inference_controller_phase8c_batch1.py -v

exit_code=$?

if [ $exit_code -eq 0 ]; then
    echo "✅ All Phase 8C Batch 1 tests passed successfully!"
else
    echo "❌ Some Phase 8C Batch 1 tests failed. Check the logs above."
fi
